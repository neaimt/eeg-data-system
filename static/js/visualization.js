import { paintGraph } from "/static/js/paintGraph.js";
import { paintFFT } from "/static/js/paintFFT.js";
...
$(document).ready(function () {
    ...
    fetchFileMetadata(file, data_id, setting, useFFT);

    hideLoading()
});

function dataLoadandVisualization(metadata) {
    ...
    fetchDataAndVisualize(beforePath)
}

// 데이터 이름으로 메타데이터 불러오기
function fetchFileMetadata(fileName, dataId, steps, useFFT) {
    // API URL 생성
    ...
    fetch(apiUrl, {
        ...
        .then(data => {
            if (data.metadata) {
                dataLoadandVisualization(data.metadata);
                console.log("파일 메타데이터:", data.metadata);
                sendEEGToServer(data.metadata, dataId, steps, useFFT);
            } else if (data.error) {
...
function sendEEGToServer(metadata, dataId, steps, useFFT) {
    // 서버로 전송할 데이터 생성
    const requestData = {
        id: dataId,
        ...
        success: function (response) {
            ...
            if (response.processed_data) {
                if (useFFT) {
                    ...
                    paintFFT(response.processed_data, "#after-visualization");
                } else {
                    console.log("일반 데이터를 시각화합니다.");
                    paintGraph(response.processed_data, "#after-visualization");
                }
...
function fetchDataAndVisualize(beforePath) {
    // before 데이터 가져오기
    $.get(beforePath)
        .done(function (beforeResponse) {
            ...
            try {
                ...
                paintGraphToTarget("#before-visualization", beforeJSON);
            } catch (error) {
...
// 지정된 타겟에 그래프를 그리는 함수
function paintGraphToTarget(targetSelector, data) {
    ...
    paintGraph(JSON.stringify(data), targetSelector); // JSON 데이터로 변환 후 전달
}
